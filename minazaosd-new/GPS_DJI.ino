...
static float heading_average = 0.0;
static byte headings_read = 0;
...
void parse_dji_gps(struct DJI_GPS *gps)
{
	...
	// calculate heading
        // ALEX-TEST: calulate heading from GPS if Groundspeed > 5km/h (1.3 m/s)
        if (GPSValues.Groundspeed > 1.3) {
          float heading = atan2(velE, velN) * 180.0 / M_PI;
	  if (heading < 0.0) heading += 360.0;
          GPSValues.Heading	= heading;
          osd_heading_source    = 0;
          headings_read = 0; // Reset the mag compass counter as soon as a GPS heading was read
          heading_average = 0.0;
        }
}
...
void parse_dji_mag(struct DJI_MAG *mag)
{
	  ...
	  if (heading < 0.0) heading += 360.0;
        ...
        // Take the average of 5 readings to smooth it a bit
          if (headings_read < 5) {
            heading_average = heading + heading_average;
            headings_read++;
          } else {
            if (GPSValues.Groundspeed <= 1.3) {
              GPSValues.Heading	= heading_average / 5;
              osd_heading_source    = 1;
            }  
            headings_read = 0;
            heading_average = 0.0;
          }
